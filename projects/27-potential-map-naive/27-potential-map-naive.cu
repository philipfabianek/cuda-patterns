...
constexpr int blocks_x = (grid_size.x + threads_per_block.x - 1) / threads_per_block.x;
 ...
 * - Each thread computes the potential for a single grid point.
 ...
 */
__global__ void potential_kernel(const float *d_atom_data, float *d_potential_map)
{
  ...
  if (x < grid_size.x && y < grid_size.y && z < grid_size.z)
  {
    ...
    d_potential_map[index] = potential;
  }
}
...
int main()
{
  ...
  CUDA_CHECK(cudaMemcpy(d_atom_data, h_atom_data.data(), atom_data_memsize, cudaMemcpyHostToDevice));
  ...
  potential_kernel<<<num_blocks, threads_per_block>>>(d_atom_data, d_potential_map);
