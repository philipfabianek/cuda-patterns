...
// Using this coarse factor also significantly improves performance
// compared to the previous version (it is 2-3 times faster),
// this specific coarse factor seems to be the best on my GPU
constexpr int coarse_factor = 128;
constexpr int blocks_per_grid = (N + threads_per_block * coarse_factor - 1) /
                                (threads_per_block * coarse_factor);

/*
 * Computes a histogram of character frequencies.
 * - Each thread processes <coarse_factor> input characters.
